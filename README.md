...
- **Roam query integration:** Better Tasks now lives inside Roam's own query surfaces. Type `{{bt-query: status="TODO" due="this-week"}}` in any block and it becomes a live task list — real Roam blocks with working checkboxes, clickable refs and inline pills, plus a result count and refresh button (filters: `status`, `project`, `due`, `completed`, `blocked`, `assignee`, `query`, `limit`, `sort`). Project, waiting-for and context values are now written as `[[page links]]`, so native `{{query}}` blocks and Datalog find your tasks with no extension involvement — and each value gets a real page with linked references. Inline pills also render on **every** copy of a task now, including native query results, `{{embed}}`s and linked references, not just the first one on the page. Full guide with copy-paste Datalog snippets: [docs/query-cookbook.md](docs/query-cookbook.md). Existing tasks are untouched; re-save an attribute to upgrade it, or turn page links off in settings.
...
- **Task deletion:** delete a task — including its whole subtree of child blocks, nested subtasks, and activity history — straight from the dashboard ⋯ menu, the bulk-action bar (multi-select), the inline pill ⋯ menu, or the `d` key. Always asks for confirmation first (with a subtask-count warning when the task has children), then shows an Undo toast that restores the task exactly as it was — same block refs, metadata, and dependency links. One limitation: a `((block ref))` you typed into ordinary text elsewhere is flattened to plain text by Roam on delete and cannot be brought back by Undo (references managed by Better Tasks — dependencies and explicit subtask links — are cleaned first and fully restored). Also available programmatically via `bt_delete` / `bt_bulk_delete`, which require `confirm: true`.
...
- **Keyboard navigation:** vim-style dashboard shortcuts — j/k to navigate, Enter to open, c to complete, s/S to snooze (+1d/+7d), d to delete (confirm dialog), e to expand subtasks, . to open the task menu (with arrow/j/k navigation), x to select, / to search, f for full-page, r to refresh, ? for shortcut legend. Bindings are customisable via JSON in Advanced Dashboard settings. All actions are bulk-aware when tasks are selected.
...
- Inline pills in query results — render pills on every copy of a task (query results, embeds, linked references), not just the first on the page
- Write Project/Waiting/Context as page links — stores those values as `[[page links]]` so native Roam queries find your tasks; creates pages and linked references for them
- `{{bt-query}}` task lists — render interactive result lists in blocks containing `{{bt-query: …}}`
